...
<body>
    ...
    <div id="pageTitle">Moje ankiety</div>
    
    <!--------------------------------CONTENT-------------------------------->
    <div class="content">
        <?php
            require('D:\OneDrive\Programowanie\PHP\XAMPP\htdocs\Serwis Ankietowy\classes\Survey.php');
            session_start();
            if ($_SESSION['UserSurveys'] == 0){
                echo $_SESSION['NoSurveys'];
                unset($_SESSION['NoSurveys']);
            }
            else{
                for($i = 0; $i < sizeof($_SESSION['UserSurveys']); $i++){
                    echo "<div>".$_SESSION['UserSurveys'][$i]->getSurveyName()."</div>";
                    echo "<div>".$_SESSION['SurveyUrls'][$i]."</div>";
                } 
            }
        ?>
    </div>
